#!/usr/bin/env tsx
/**
 * Database builder for Togo Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Togo Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Togolese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawtogolese.com');
