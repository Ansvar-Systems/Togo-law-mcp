#!/usr/bin/env tsx
/**
 * Togolese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawtogolese.com';
const PORTAL_NAME = 'Togo Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/togolese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Togolese Law MCP — Data Freshness Check');
