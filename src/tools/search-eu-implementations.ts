/**
 * search_eu_implementations — Search EU directives/regulations that have Togolese implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_togolese_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as togolese_statute_count
  ...
  if (input.has_togolese_implementation) {
    sql += ' HAVING togolese_statute_count > 0';
  }
