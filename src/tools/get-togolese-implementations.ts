/**
 * get_togolese_implementations — Find Togolese statutes implementing a specific EU directive/regulation
...
export interface GetTogoleseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface TogoleseImplementationResult {
  document_id: string;
...
export async function getTogoleseImplementations(
  db: InstanceType<typeof Database>,
  input: GetTogoleseImplementationsInput,
): Promise<ToolResponse<TogoleseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as TogoleseImplementationResult[];
